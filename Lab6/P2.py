...
class WeatherTracker(QMainWindow):
    ...
    def _build_ui(self):
        ...
        stats_lbl.setFont(QFont("Georgia", 12, QFont.Bold))
    ...
    def _display(self, df: pd.DataFrame):
        ...
        try:
            # replace QTableWidget() with a function that gets the stat table
    ...
    def _on_read_csv(self):
        ...
        try:
            # replace pd.DataFrame() with calling a function to get a data frame
    ...
    def _on_add(self):
        ...
        record = {
        "date":         self.in_date.date().toString("yyyy-MM-dd"), # QDateEdit คืน QDate ต้องแปลงเป็น str ก่อน
        "city":         self.in_city.currentText(),
        "temp_c":       self.in_temp.value(), # QDoubleSpinBox ใช้ .value() ได้เลย คืน float
        "humidity":     self.in_hum.value(),
        "rainfall_mm":  self.in_rain.value(),
        "condition":    self.in_cond.currentText(),
        }

        # put the record{} into a DataFrame(DF)
        new_row = pd.DataFrame([record])

        # concat the new row to the current Main DF
    ...
    def _on_chart(self):
        ...
        try:
            # replace QWidget() with calling a function to create a graph widget
            widget = show_chart(self.df, "rainfall")
