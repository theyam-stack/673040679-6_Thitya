...
class TaskCard(QFrame):
    ...
    def _build_ui(self):
        ...
        lbl_dead.setStyleSheet(" color:#222;")
...
class AddTaskDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QLineEdit {
                ...
                padding:6px 10px; font-size:10pt; background:#fff; color:#333;
            }
            QLineEdit:focus { border-color:#4a90d9; }
            QComboBox {
                border:1.5px solid #ced4da; border-radius:8px;
                padding:5px 10px; font-size:10pt; background:#fff; color:#333;
            }
            QCalendarWidget {
                border:1.5px solid #ced4da; border-radius:8px;
            }
    ...
    def _build_ui(self):
        ...
        btn_cancel.setStyleSheet(
            "QPushButton{background:#EA7B7B;border:none;border-radius:8px;"
            "font-size:10pt;color:#fff; }"
            "QPushButton:hover{background:#A82323;}"
        )
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.lbl_count.setStyleSheet("color:#555; font-size:10pt;")
        ...
        self.lbl_empty.setStyleSheet("color:#000; font-size:12pt;")
    ...
    def _add_task(self):
        ...
        if dlg.exec() == QDialog.Accepted:
            # Get task from dialog and add it to the DB
            task = dlg.get_task()
            ...
            # Update the task count and check whether card area is empty
            self._refresh_count()
            self._refresh_empty()
    ...
    def _remove_card(self, card: TaskCard):
        card.task["done"] = True   # Mark done in DB
    ...
    def _load_json(self):
        ...
        if filename:
            try:
                # Load chosen file and get data
                with open(filename, 'r', encoding='utf-8') as f:
                    loaded_tasks = json.load(f)
                
                # Clear existing tasks and cards
                ...
                QMessageBox.information(self, "Loaded", f"Loaded {len(self.tasks)} active tasks.")
